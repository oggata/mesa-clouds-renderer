...
function make(CELL, CHAR_SCALE, env) {
  ...
  return {
    ...
    //
    // ★ CAR_VIS … 見た目だけの倍率。**実寸ではない**ので meters の欄はそのまま
    //   軽自動車のサイズで書き、最後にここを掛ける。実寸の軽自動車を置くと、
    //   1セル 7.7m の生活道路では車が道いっぱいに見えて重く感じる (実際そう
    //   見えていた) ので、0.80 倍に落として道の余白を残す。実寸に戻したければ
    //   CAR_VIS=1 で起動する。**車間距離もこの寸法から引くので、ここを変えると
    //   traffic.js の追従距離も自動で追従する** (traffic.js に生の数字は無い)。
    CAR: (()=>{ const v = num('CAR_VIS', 0.80), w = m => wu(m) * v; return {
      vis:   v,
      len:   w(num('CAR_LEN_M',   3.30)),
      wid:   w(num('CAR_WID_M',   1.46)),
      bodyZ: [w(0.32), w(num('CAR_BODY_TOP_M', 0.95))],   // 車体 (床〜ベルトライン)
      cabZ:  [w(0.95), w(num('CAR_TOP_M',      1.58))],   // キャビン
      wheelR: w(0.27), wheelW: w(0.17),
    }; })(),
    ...
    // ── 信号機 ──────────────────────────────────────────────────────────────
    // 交差点の対角 (進行方向の向こう側) に立てる縦柱 + 車道へ張り出す腕 + 横型の灯器。
    // 日本の車両用信号は灯器が横に3つ (左から 青・黄・赤)。実物は灯火の径 30cm、
    // 灯器の幅 1.05m、路面から灯器の下端まで 5.0m だが、1セル 7.7m の生活道路に
    // 5m の信号を立てると街灯 (4.6m) より高くなって目立ちすぎるので、少し下げる。
    ...
    SIGNAL: {
      ...
      headH:   wu(0.40) / 2,                       // 灯器の高さの半分
      lensR:   wu(0.30) / 2,                       // 灯火の半径
      lensGap: wu(0.30),                           // 灯火の間隔 (中心間)
      visor:   wu(0.12),                           // 庇の出 (灯火の手前へ)
    },
...
function report(S, CELL) {
  ...
  row('信号 高さ', S.SIGNAL.h); row('信号 灯器の幅', S.SIGNAL.headW * 2);
